...
以 Qwen3-VL-2B-Instruct 为基座，通过 LoRA 高效微调（仅训练 3% 参数）、数据清洗与 DPO 偏好对齐，在 RSICD 基准上达到与专用架构相当的水平。全程使用单张 RTX 3060 12GB 消费级显卡完成。
...
| **Ours (DPO 对齐)** | **通用 VLM + 3% LoRA** | **31.5** | **53.2** | **108.6** |
| Adaptive | 专用架构 | 30.1 | 53.6 | 113.9 |
| SAT | 专用架构 | 28.0 | 49.5 | 87.0 |
| Ours (数据清洗 SFT) | 通用 VLM + 3% LoRA | 28.4 | 50.8 | 62.4 |
| Ours (原始 SFT) | 通用 VLM + 0.8% LoRA | 21.2 | 46.6 | 1.1 |
| Qwen3-VL-2B Zero-shot | 通用 VLM | 0.8 | 3.0 | 0.0 |

> **核心发现**：通用 VLM 在 zero-shot 下完全无法完成遥感图像描述（75% 空输出），但通过针对性微调，仅训练 3% 参数即可达到专用架构的水平。CIDEr 从 0.0 提升至 108.6，证明了 VLM + 高效微调在遥感领域的可行性与潜力。
...
Qwen3-VL-2B              + LoRA r=16              + Visual LoRA           + 偏好训练
    │                        │                        │                      │
    ├─ 75% 空输出             ├─ 学会任务格式            ├─ 数据去重清洗           ├─ 突破 SFT 天花板
    ├─ 25% 聊天式回复         ├─ 但 CIDEr=1.1           ├─ 视觉编码器适配          ├─ 主动选择区分性词汇
    └─ CIDEr=0.0             └─ 只输出模板句             └─ CIDEr 1.1→62.4       └─ CIDEr 62.4→108.6
```
...
|------|--------|------------|------|
| 视觉编码器 | 24× ViT blocks | `qkv`, `proj`, `linear_fc1`, `linear_fc2` | 16 |
...
| 数据去重清洗 | +7.2 | +61.3 | 消除多参考"最小公分母"效应 |
| 视觉编码器 LoRA | 内嵌 | 内嵌 | 航拍视角的视觉特征适配是关键 |
| DPO 偏好对齐 | +3.1 | +46.2 | 负信号突破交叉熵天花板 |
