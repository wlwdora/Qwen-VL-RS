...
v2 (数据清洗+SFT)    BLEU-4=28.8   CIDEr=  66.0   ← 数据清洗 + ViT r=32, LLM r=32
v3 (DPO, ViT r=64)  BLEU-4=33.2   CIDEr= 126.5   ← 视觉域迁移 + 偏好对齐双轮驱动
```
...
**深化反思 — ViT 应该用更高的 rank**：

修复后发现 ViT 仅给 r=16 效果提升有限。进一步分析：遥感图像描述**最大的瓶颈在视觉端而非语言端**。

| 维度 | ViT | LLM |
|------|-----|-----|
| 预训练域 | 自然图像（地面视角） | 通用英文文本 |
| 目标域 | 航拍遥感（俯视） | 简洁描述句 |
| 域差距 | **极大** — 视角/纹理/目标完全不同 | **小** — 只需学领域词汇 |
| 需要学的 | 全新的视觉特征体系 | 几十个新词 + 固定句式 |

ViT 的 24 个 block 从低层边缘检测到高层语义理解，全部需要适应遥感特征。r=16 的表示容量远远不够。最终方案：**ViT r=64, LLM r=32**——将参数投向域差距最大的地方。LoRA 参数本身仅 ~190MB，对 12GB 显存几乎无影响，rank 选择完全取决于表示容量需求而非显存。
...
|--------|-------------------|------------|
| Visual encoder | 全部冻结 | LoRA r=32 |
| LLM | LoRA r=16 | LoRA r=32 |
| 训练数据 | 原始 5 参考/图 | 清洗后 1-2 条/图 |
| 可训练参数 | 17.4M (0.81%) | ~75M (~3.5%) |

| 方法 | BLEU-4 | ROUGE-L | CIDEr-D |
|------|--------|---------|---------|
| Transformer (SOTA) | 35.7 | 58.2 | 155.1 |
| SAT | 28.0 | 49.5 | 87.0 |
| **Ours v2** | **28.8** | **51.0** | **66.0** |
| Ours v1 | 21.2 | 46.6 | 1.1 |

**分析**：
- CIDEr 1.1→66.0 (+64.9)：数据清洗消除了"最小公分母"，所有训练参考都是区分性描述；ViT r=32 让视觉编码器能提取遥感特有特征
- BLEU 21.2→28.8：超过 SAT (28.0)
- ROUGE-L 46.6→51.0：超过 SAT (49.5)

**剩余天花板**：CIDEr 66.0 vs GCN-LSTM 138.6 仍差 ~2×。两个瓶颈：① ViT r=32 对遥感域迁移仍不够（见 Error 10 反思）；② 交叉熵训练只给"正面信号"。→ Error 10 反思提出 ViT r=64，Error 13 用 DPO 突破交叉熵天花板。
...
v3 最终配置（结合 Error 10 的反思）：**ViT r=64 + LLM r=32 + 数据清洗 + DPO**。

| 方法 | BLEU-4 | ROUGE-L | CIDEr-D |
|------|--------|---------|---------|
| Transformer (SOTA) | 35.7 | 58.2 | 155.1 |
| GCN-LSTM | 33.4 | 56.5 | 138.6 |
| **Ours v3 (DPO, ViT r=64)** | **33.2** | **55.1** | **126.5** |
| Up-Down | 31.3 | 54.5 | 124.8 |
| Adaptive | 30.1 | 53.6 | 113.9 |
| SAT | 28.0 | 49.5 | 87.0 |
| Ours v2 (SFT, ViT r=32) | 28.8 | 51.0 | 66.0 |
...
```
v1→v2: 1.1 → 66.0    (+64.9)   数据清洗修复"参考端"，ViT r=32 开启视觉适配
v2→v3: 66.0 → 126.5  (+60.5)   ViT r=64 + DPO，视觉域迁移 + 偏好对齐双轮驱动
```

**结论**：
- CIDEr 从 0.0 到 126.5，达到 GCN-LSTM（遥感专用架构 SOTA 之一）的 91%
- BLEU-4 33.2 接近 GCN-LSTM (33.4)，超过了 Up-Down (31.3)
- 核心发现：**遥感 VLM 微调的核心矛盾在视觉端**——ViT 从自然图像到航拍的域迁移是最难的环节。r=64 的高 rank 视觉 LoRA + 数据清洗 + DPO 的组合，让通用 2B VLM 达到了专用架构的水平
- 核心方法论：**识别核心域差距（视觉）→ 精准分配 rank → 数据清洗 → 偏好对齐**
...
4. **遥感 VLM 的核心矛盾在视觉端** — ViT 从自然图像到航拍的域迁移远比 LLM 学新词困难，rank 应优先投向视觉编码器（ViT r=64 vs LLM r=32）
5. **SFT 有天花板** — 交叉熵只给正面信号，区分性描述需要 DPO 偏好对齐来解锁
